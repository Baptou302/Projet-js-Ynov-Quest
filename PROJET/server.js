...
const query = (sql, params) => pool.getConnection()
...
app.post("/login", async (req, res) => {
    try {
        ...
        res.json(ok
            ? { success: true, userId: users[0].id, firstname: users[0].pseudo }
...
app.post("/register", async (req, res) => {
    try {
        ...
        const [result] = await query(
            "INSERT INTO users (pseudo, firstname, lastname, email, password_hash) VALUES (?, ?, ?, ?, ?)",
            [pseudo, firstname, lastname, email, hash]
        );
