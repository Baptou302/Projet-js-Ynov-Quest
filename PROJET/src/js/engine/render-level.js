...
const types = {
    ...
    text: () => `<input id="answer" class="input" placeholder="Ta réponse"><button class="btn" data-action="submit">Valider</button>`
};
...
function renderLevel(level) {
    ...
    return `<section class="screen stack">
        ...
        <h2>${level.title}</h2><p>${level.intro}</p>
        <div class="card stack"><h3>${level.course.title}</h3><ul>${level.course.points.map(p => `<li>${p}</li>`).join('')}</ul></div>
