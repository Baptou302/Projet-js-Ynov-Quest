function renderWorld(worlds) {
  ...
  <div class="world-list">
    ${worlds.map(w => `<article class="world-card" data-action="selectWorld" data-value="${w.id}"><div class="world-circle ${w.id}"><img src="${w.image}" alt="${w.name}"></div><h3>${w.name}</h3></article>`).join('')}
  </div>
